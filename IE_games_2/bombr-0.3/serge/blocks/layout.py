...
class BaseGrid(Container):
    ...
    def getCellFromLocation(self, (x, y)):
        ...
        if dx < 0 or dx >= w or dy < 0 or dy >= h:
            raise ValueError('The point is outside the range of the grid')
        #
        return int(dx / w * self.cols), int(dy / h * self.rows)
...
class Grid(BaseGrid):
    ...
    def addActor(self, (x, y), actor, layer_name=None):
        ...
        try:
            ...
            raise CellOccupied('The cell %s is already occupied by %s in grid %s' %
...
class ManualUpdateGrid(Grid):
    """A grid where the actors do not render themselves automatically

    This grid is used as an optimization where you have a grid of actors
    that rarely change their appearance. The grid will render them once
    to a large surface.

    When they change you call the updateRendering and the entire grid will
    refresh. This means that each frame you are not updating the rendering
    of all the actors and can just write out the pre-rendered image.

    The grid implements this by adjusting the "visible" attribute of each
    actor so do not change this.
    ...
    def __init__(self, tag, name='', size=(1, 1), width=None,
                 height=None, background_colour=None, background_layer=None):
        """Initialise the Grid"""
        super(ManualUpdateGrid, self).__init__(tag, name, size, width, height, background_colour, background_layer)
        #
    ...
    def addActor(self, (x, y), actor, layer_name=None):
        ...
        """
        result = super(ManualUpdateGrid, self).addActor((x, y), actor, layer_name)
        #
        actor.visible = False
        return result
    ...
    def updateRendering(self):
        ...
        for actor in self.getChildren():
            actor_left, actor_top, width, height = actor.getSpatial()
            actor.visual.renderTo(0, surface, (actor_left - left, actor_top - top))